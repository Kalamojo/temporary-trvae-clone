...
if specific_celltype == 'all':
    for specific_celltype in adata.obs[cell_type_key].unique().tolist():
        ...
        model.train(net_adata, condition_key=condition_key, cell_type_key=cell_type_key, n_epochs=300, patience=10,
                    lr_reducer=7)
...
#                               path_to_save=f"./results/{data_name}/", model_name="scVI_latent")
        ...
        pred_adata.write_h5ad(f"./data/reconstructed/scVI_{data_name}/{specific_celltype}.h5ad")
else:
    net_adata = adata[
        ~((adata.obs[cell_type_key] == specific_celltype) & (adata.obs[condition_key].isin(target_conditions)))]
