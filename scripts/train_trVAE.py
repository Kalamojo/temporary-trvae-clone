...
if specific_celltype == 'all':
    ...
    for specific_celltype in adata.obs[cell_type_key].unique().tolist():
        ...
        #                       ext="png")
elif specific_celltype == 'c3m':
    indices = np.arange(adata.shape[0])
    train_idx = np.concatenate([indices[:0], indices[0:2], indices[8:10], indices[16:]])
    test_idx = np.concatenate([indices[2:8], indices[10:16]])
